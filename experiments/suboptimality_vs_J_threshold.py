...
def compute_J_linspace(s_list, shat_list, n_points):
    ...
    return np.linspace(max(max(lowJs),0), min(upJs), n_points)

def compute_betas(s, shat):
    lb_sat = 1/(max(s,shat))
    ub_sat = 1/(min(s,shat))
    lowbeta = max(0.2*lb_sat, 1)
    midbeta = (lowbeta+ub_sat)/2
    highbeta = 1.2*ub_sat
...
if __name__ == "__main__":
    ...
    for J in Js:
        for i, (model_id, s, shat, true_df) in enumerate(zip(model_ids, s_list, shat_list, true_dfs)):
            ...
            betas = compute_betas(s, estimated_shat)
            #print("- beta linspace:", betas, "\n")

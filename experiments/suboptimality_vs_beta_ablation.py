...
if __name__ == "__main__":
    ...
    for i, (model_id, s, shat, true_df) in enumerate(zip(model_ids, s_list, shat_list, true_dfs)):
        ...
        # Compute
        for s_used in [1]: #np.linspace(0,1,10):
            for beta in tqdm(betas, desc=f"{model_id} - J: {J} - shat {shat}"):
                ...
                for mode, df in dfs.items():
                    ...
                    for t in trange(num_episodes, desc="SRS", leave=False):
                        res = rejection_sampling(df, beta, s_used, rng=rng, max_iter=1e6)
                    ...
                    for t in trange(num_episodes, desc="SMC", leave=False):
                        res = maximal_coupling(df, beta, s_used, rng=rng, max_iter=1e6)
                        reward = compute_reward(res["sample_id"])
                        rewards.append(reward)
                        iters.append(res["iter"])
                    results = {
    ...
    new_dir = os.path.join(ROOT, f"tasks/{task}/results-{sample_id}/suboptimality_vs_beta_ablation_s")
    os.makedirs(new_dir, exist_ok=True)
    result_df_path = os.path.join(new_dir, f"{task}-{model_names_flat}-episodes{num_episodes}-i{sample_id}-ablation-debug.csv")

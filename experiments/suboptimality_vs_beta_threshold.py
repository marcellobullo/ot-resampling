...
from algorithms import rejection_sampling, maximal_coupling, AiC
...
if __name__ == "__main__":
    ...
    for i, (model_id, s, shat, true_df) in enumerate(zip(model_ids, s_list, shat_list, true_dfs)):
        ...
        # Compute
        for beta in tqdm(betas, desc=f"{model_id} - J: {estimated_J} - shat {estimated_shat} - th {threshold}"):
    ...
    result_df_path = os.path.join(new_dir, f"{task}-{model_names_flat}-episodes{num_episodes}-i{sample_id}-th.csv")

...
from sampling_utils import get_df, get_estimated_df, get_youden_bounds, compute_s, get_membership_fn, batched_rejection_sampling, BoN
...
if __name__ == "__main__":
        ...
        for beta in tqdm(betas, desc=f"{model_id} - J: {J} - shat {shat}"):
            ...
            for mode, df in dfs.items():
                ...
                for batch_size in batch_sizes:
                    rewards, iters = [], []
                    for t in trange(num_episodes, desc=f"BRS - MODE: {mode} - N={batch_size} - BETA: {beta}", leave=False):
    ...
    result_df_path = os.path.join(new_dir, f"{task}-{model_names_flat}-episodes{num_episodes}-i{sample_id}-BRS-BoN.csv")
